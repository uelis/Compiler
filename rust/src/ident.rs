...
impl Ident {
    pub fn new() -> Ident {
        NEXTIDENT.with(|nextident| {
            let mut n = nextident.borrow_mut();
            *n += 1;
            Ident { id: *n }
        })
    }
...
impl Name {
    ...
    pub fn new() -> Name {
        with_interner(|interner| Name::Interned(interner.fresh_id()))
    }
    ...
    pub fn named(s: &str) -> Name {
        with_interner(|interner| Name::Interned(interner.id_of_str(s)))
    }
}
...
impl Display for Name {
    fn fmt(&self, fmt: &mut Formatter) -> Result<(), Error> {
        match *self {
            Name::Interned(i) => with_interner(|interner| match interner.str_of_id(i) {
                None => write!(fmt, "t{}", i),
                Some(s) => write!(fmt, "{}", s),
            }),
            Name::Static(s) => write!(fmt, "{}", s),
        }
    }
}
...
// Each tread stores an interner.
thread_local!( static NEXTIDENT: RefCell < usize > = RefCell::new(0));
