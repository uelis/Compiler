...
fn canonize_stm(s: Stm) -> Vec<Stm> {
    use intermediate::tree::Exp::*;
    use intermediate::tree::Stm::*;
    match s {
        Move(e1, e2) => match e1 {
            Mem(a) => {
...
fn canonize_exp(e: Exp) -> Block {
    ...
    match e {
        ...
        BinOp(o, e1, e2) => {
            let b1 = canonize_exp_no_top_call(*e1);
            let b2 = canonize_exp_no_top_call(*e2);
            Block::combine(b1, b2, &|e1, e2| binop!(o, e1, e2))
        }
        Call(e, mut args) => {
...
impl Block {
    ...
    fn combine(b1: Block, mut b2: Block, f: &dyn Fn(Exp, Exp) -> Exp) -> Block {
        let mut b = Block::compose(b1, &mut b2.stms);
        b.exp = f(b.exp, b2.exp);
        b
    }

    fn combine_n(b: Block, mut bs: Vec<Block>, f: &dyn Fn(Exp, Vec<Exp>) -> Exp) -> Block {
        bs.reverse();
        let mut ss = vec![];
        let mut es = vec![];
        for b in bs {
    ...
    fn combine_stm(b1: Block, mut b2: Block, f: &dyn Fn(Exp, Exp) -> Stm) -> Vec<Stm> {
        let mut b = Block::compose(b1, &mut b2.stms);
