...
#[derive(Debug)]
pub struct Prg {
    pub names: NamingContext,
    pub functions: Vec<Function>,
}

macro_rules! call {
    ($e:expr, $f:expr) => {
        Exp::Call(Box::new($e), vec![$f])
    };
}

macro_rules! binop {
    ($e:expr, $f:expr, $g:expr) => {
        Exp::BinOp($e, Box::new($f), Box::new($g))
    };
}

macro_rules! jump {
    ($e:expr) => {
        Stm::Jump(Exp::Name($e), vec![$e])
    };
}

impl RelOp {
    pub fn neg(&self) -> RelOp {
        use intermediate::tree::RelOp::*;
        match *self {
            EQ => NE,
            NE => EQ,
            LT => GE,
...
impl fmt::Display for Exp {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        ...
        match *self {
            ...
            Name(ref x) => write!(fmt, "NAME({})", x),
            Temp(ref x) => write!(fmt, "TEMP({})", x),
            Param(i) => write!(fmt, "PARAM({})", i),
            Mem(ref e) => write!(fmt, "MEM({})", e),
            BinOp(ref op, ref e1, ref e2) => write!(fmt, "BINOP({}, {}, {})", op, e1, e2),
...
impl fmt::Display for Stm {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        use self::Stm::*;
        match *self {
            Move(ref e1, ref e2) => write!(fmt, "MOVE({}, {})", e1, e2),
            Jump(ref e, ref is) => {
                write!(fmt, "JUMP({}", e)?;
                for i in is {
                    write!(fmt, ", {}", i)?;
